...
PUMP_SELECTOR.LoadNX    =   function ( speed, songid, mode, level, location )    {
	console.log("Loading song: ",speed, songid, mode, level, location);
	LoadSongRank(songid, mode, level);
...
PUMP_SELECTOR.Game = PUMP_SELECTOR.Game || function ( parameters ) {
    ...
    if(userid === undefined || userid.length == 0)
    	userid = "HUEBR TEAM";
    $.getJSON( "?page=api&cmd=getsteplist&userid="+userid, function( data ) {
        _this.songlist = data.data;
        console.log("PUMP_SELECTOR::Game::Loaded songlist. "+data.data.length+" songs.");
